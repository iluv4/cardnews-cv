...
def ablation_table():
    ...
    return KeepTogether([Spacer(1, 2), t,
                         P("[표 1] 시드 109장(의사 레이블) 기준 검출기 ablation.", caption)])
...
def build():
    ...
    s.append(P("<b>표준화.</b> 모든 이미지는 ASCII 파일명의 RGB JPEG로 재인코딩하여"
               ...
               "추가로 513×750 PosterLayout 캔버스로 리스케일한다. 표본은 [그림 1].", body0))
    s.append(figure("fig_dataset.png",
                    "[그림 1] 687장·66덱 한국어 카드뉴스 코퍼스의 대표 페이지."))
    s.append(P("<b>자동 라벨링.</b> 수작업 박스 주석은 확장성이 없으므로 <i>의사 레이블</i>을 생성한다. "
               ...
               "logo/underlay는 수작업 주석 대상으로 남긴다.", body))

    # ===== 4. Method =====
    s.append(P("4. 방법", h1))
    s.append(P("[그림 2]는 시스템을 요약한다: 수집 → 검출 → {템플릿 엔진 | DS-GAN} → 렌더링.", body0))
    s.append(figure("fig_pipeline.png",
                    "[그림 2] 시스템 개요. 공유 검출기가 두 생성 경로에 입력을 공급한다. "
    ...
    s.append(P("[그림 3]은 일관된 <b>표지 대 내지</b> 패턴을 보인다: 표지는 뚜렷하게 더 <i>어둡고</i>"
    ...
    s.append(figure("fig_timeseries.png",
                    "[그림 3] 상대 페이지 위치에 따른 덱 동역학(66덱). 표지는 내지보다 더 어둡고 "
    ...
    s.append(P("[표 1]과 [그림 4]가 ablation을 보고한다. 전이학습이 지배적 요인이다(scratch 0.502 대 "
               "baseline 0.662). 카드에 맞춘 증강과 더 긴 스케줄이 최고의 nano 모델을 준다"
               "(long300_card: mAP@50-95 = 0.718, mAP@50 = 0.854). YOLOv8-small도 경쟁력 있다(0.705). "
               "시드 반복 간 편차는 <0.02 mAP로 순위가 안정적이다. 정성적 검출은 [그림 5].", body0))
    s.append(ablation_table())
    s.append(figure("fig_ablation.png",
                    "[그림 4] 주요 ablation 결과(박스 mAP@50-95). 전이학습과 카드 맞춤·장기 스케줄이 "
                    "향상을 견인한다."))
    s.append(figure("fig_detector_pred.jpg",
                    "[그림 5] 검증용 미사용 카드에 대한 검출기 예측(제목/본문).", max_h=86*mm))
    ...
    s.append(P("[그림 6]은 템플릿 엔진 출력과 복사-재렌더링 예시(원본 → 정리된 배경 → 재구성)를 보인다. "
               "템플릿 엔진은 1080×1350에서 읽기 쉽고 일관되게 디자인된 한글 카드를 산출하는 반면, DS-GAN "
               "경로는 쓸 만한 <i>박스</i>는 만들지만 배포 가능한 렌더링 텍스트는 만들지 못한다. 따라서 "
               "실서비스용 카드뉴스에는 템플릿 엔진을 채택하고 DS-GAN은 연구용 비교로 남긴다.", body0))
    s.append(figure("fig_results_template.png",
                    "[그림 6a] 템플릿 엔진 카드(고해상도, Pretendard, 적응형 색상)."))
    s.append(figure("fig_results_copy.png",
